...
query = "SELECT a.first_name, a.last_name, b.title, p.name FROM books AS b LEFT JOIN publishers AS p ON p.publisher_id = b.publisher_id LEFT JOIN book_authors AS ba ON ba.book_id = b.book_id LEFT JOIN authors AS a ON a.author_id = ba.author_id ORDER BY b.title ASC"
